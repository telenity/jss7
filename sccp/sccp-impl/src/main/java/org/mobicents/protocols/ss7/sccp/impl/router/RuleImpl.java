...
public class RuleImpl implements Rule, Serializable {
    ...
    private char[] patternDigitsChars;
    ...
    private void configure() {
        ...
        this.patternDigitsChars = this.pattern.getGlobalTitle().getDigits().toCharArray();
    }
    ...
    public boolean matches(SccpAddress address, boolean isMtpOriginated) {
        ...
        AddressIndicator addressIndicator = address.getAddressIndicator();
        ...
        char[] addressDigitsChars = address.getGlobalTitle().getDigits().toCharArray();
        switch (addressIndicator.getGlobalTitleIndicator()) {
            case GLOBAL_TITLE_INCLUDES_NATURE_OF_ADDRESS_INDICATOR_ONLY:
                ...
                // digits must match
                if (!matchPattern(addressDigitsChars, this.patternDigitsChars)) {
                    if (logger.isInfoEnabled()) {
