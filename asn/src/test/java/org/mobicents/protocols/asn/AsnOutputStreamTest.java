 ...
 */
public class AsnOutputStreamTest {
    ...
    private void compareArrays(byte[] expected, byte[] encoded) {
        ...
        assertTrue("byte[] don't match, expected|encoded \n"
                        + Arrays.toString(expected) + "\n" + Arrays.toString(encoded),
                same);
    }

    @Test
    public void testInitialLengthConstructor() throws Exception {
        AsnOutputStream sizedOutput = new AsnOutputStream(3);
        byte[] expected = new byte[]{1, 2, 3};

        sizedOutput.write(expected);

        assertEquals(3, sizedOutput.size());
        compareArrays(expected, sizedOutput.toByteArray());
    }

    @Test
    public void testInitialLengthConstructorGrows() throws Exception {
        AsnOutputStream sizedOutput = new AsnOutputStream(1);
        byte[] expected = new byte[]{1, 2, 3};
    ...
    public void testInitialLengthConstructorRejectsNegativeLength() {
        new AsnOutputStream(-1);
    }
    ...
    public void testRealBinary118_625() throws Exception { // s E M
        ...
        // 00000000 00000000
        // T L V: info bits, exp(2), mantissa(7)
    ...
    public void testReal10() throws Exception {
        // we actually don't check NR, its responsibility of other
        // its base10 are encoded as strings... ech, we don't check encoded
        // string... should we?
        ...
        for (int index = 0; index < digs.length; index++) {
            ...
            // NR should change, but its responsibility of user.
            ...
            assertEquals("Decoded value is not proper!!", d, dd, 0.0d);
        }
    ...
    public void testBitString_Short() throws Exception {
        // 11110000 11110000 111101xx //0x0F according to book...
    ...
    public void testWriteSequence() throws Exception {
        ...
        this.output.writeSequence(inner);
        byte[] encoded = this.output.toByteArray();
        // UNIVERSAL + constructed(0x20) + SEQUENCE(0x10) = 0x30
        assertEquals(0x30, encoded[0] & 0xFF);
        assertEquals(4, encoded[1]);
        assertArrayEquals(inner, Arrays.copyOfRange(encoded, 2, encoded.length));

        // round-trip decode
        AsnInputStream input = new AsnInputStream(encoded);
        assertEquals(Tag.SEQUENCE, input.readTag());
        byte[] decoded = input.readSequence();
        assertArrayEquals(inner, decoded);
        assertEquals(0, input.available());
    }

    @Test
    public void testWriteSequenceWithCustomTag() throws Exception {
        byte[] inner = new byte[]{0x11, 0x22};
        this.output.writeSequence(Tag.CLASS_APPLICATION, 5, inner);
        byte[] encoded = this.output.toByteArray();
        byte tag = encoded[0];
        assertEquals(Tag.CLASS_APPLICATION, (tag & Tag.CLASS_MASK) >> 6);
        assertEquals(5, tag & Tag.TAG_MASK);
        assertEquals(2, encoded[1]);
        assertArrayEquals(inner, Arrays.copyOfRange(encoded, 2, encoded.length));
    }

    @Test
    public void testRoundTripGraphicString() throws Exception {
        String val = "GraphicString-test-123";
        this.output.writeStringGraphic(val);
        AsnInputStream input = new AsnInputStream(this.output.toByteArray());
        assertEquals(Tag.STRING_GRAPHIC, input.readTag());
        assertEquals(val, input.readGraphicString());
        assertEquals(0, input.available());
    }
}
